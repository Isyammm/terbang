...
import { Plane, ShieldAlert, ArrowRight, Eye, Trash2, Ticket, Lock, LogIn } from 'lucide-react';
...
export const BookingHistory: React.FC<BookingHistoryProps> = ({
  ...
  // If not logged in, show locked view
  if (!currentUser) {
    return (
      <div className="history-container animated-fade-in">
        ...
        <div className="empty-history glass-card text-center history-locked">
          <Lock size={52} className="empty-icon" style={{ color: 'rgba(139,92,246,0.6)' }} />
          <h3>Akses Riwayat Dibatasi</h3>
          <p>Masuk atau daftar akun FlyEase untuk melihat dan mengelola riwayat tiket penerbangan Anda.</p>
          <button
            className="btn btn-primary empty-cta-btn"
            onClick={onLoginClick}
            id="history-login-cta-btn"
          >
